...
// function prototype
void moveTower(int disks, vector<string> &source, vector<string> &destination,
...
moveTower(disks, source, destination, temporary)
    IF disks == 0, THEN:
        return
    ELSE:
        move all but one disk from source to temporary using destination as
spare move one disk from source to destination print current layout move all but
one disk from temporary to destination using source as spare END IF
*/
void moveTower(int disks, vector<string> &source, vector<string> &destination,
               vector<string> &temporary) {
